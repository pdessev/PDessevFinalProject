...
/*
 * fb[y*W+x] OR fb[y][x]
 * Alternatively, we can modify the linker script to have an end address of
 * 20013DFB instead of 2002FFFF, so it does not place variables in the same
 * region as the frame buffer. In this case it is safe to just specify the raw
 * address as frame buffer.
 */
// uint32_t frameBuffer[(LCD_PIXEL_WIDTH*LCD_PIXEL_WIDTH)/2] = {0};
// //16bpp pixel format. We can size to uint32. this ensures 32 bit alignment
...
Result LTCD_Layer_Init(uint8_t LayerIndex) {
    ...
    if (HAL_LTDC_ConfigLayer(&hltdc, &pLayerCfg, LayerIndex) != HAL_OK) {
        return result("Unable to configure LCD Layer");
    }
    return result(0);
}

void clearScreen(void) { LCD_Clear(0, LCD_COLOR_WHITE); }

Result LTCD_Init(void) {
    hltdc.Instance = LTDC;
    ...
    if (HAL_LTDC_Init(&hltdc) != HAL_OK) {
        return result("Unable to initialize LCD HAL");
    }
...
// This was taken and adapted from stm32's mcu code
void LCD_SetFont(FONT_t* fonts) { LCD_Currentfonts = fonts; }
